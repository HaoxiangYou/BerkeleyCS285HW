...
def run_training_loop(config: dict, logger: Logger, args: argparse.Namespace):
    ...
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        if isinstance(replay_buffer, MemoryEfficientReplayBuffer):
            ...
            replay_buffer.insert(action=action, reward=reward, next_observation=next_observation[-1,...],
                                done=(done and not truncated))
        else:
            # We're using the regular replay buffer
            replay_buffer.insert(observation=observation, action=action, reward=reward, 
                                next_observation=next_observation, done=(done and not truncated))
        ...
        if step >= config["learning_starts"]:
            ...
            update_info = agent.update(obs=batch["observations"], action=batch["actions"], 
                                    reward=batch["rewards"], next_obs=batch["next_observations"], done=batch["dones"], step=step)
