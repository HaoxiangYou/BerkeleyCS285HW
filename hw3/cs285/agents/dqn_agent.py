    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                next_qa_values_by_critic = self.critic(next_obs)
                next_action = torch.argmax(next_qa_values_by_critic, dim=1)
            else:
                next_action = torch.argmax(next_qa_values, dim=1)
            next_q_values = next_qa_values.gather(1, next_action.unsqueeze(1))
            target_values = self.discount * next_q_values * (1 - (done[:,None]).to(torch.float)) + reward[:,None]
